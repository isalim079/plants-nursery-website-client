...
import { useGetPlantByIdQuery, useUpdatePlantsDataMutation } from "../../redux/api/api";
    ...
    const onSubmit = async (data: Plant) => {
        ...
        if (res.data.success) {
            ...
            const plantsData = {
                ...
                quantity: Number(data.quantity ? data.quantity : plant?.data?.quantity),
                categoryName: data.categoryName ? data.categoryName : plant?.data?.categoryName,
                description: data.description ? data.description : plant?.data?.description,
            };
            ...
            <div className="max-w-screen-2xl mx-auto my-24 p-6 bg-white rounded-md shadow-md ">
                ...
                >
                    ...
                    <div className="flex justify-between">
                        <div>
                            <button type="submit" className="border px-3 py-2 border-textGreen/50 text-textGreen rounded-md shadow-md cursor-pointer  hover:bg-textGreen hover:text-[#fff] transition duration-200 ease-in-out">
